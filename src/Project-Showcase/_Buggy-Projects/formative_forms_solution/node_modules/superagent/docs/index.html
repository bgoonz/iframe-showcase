...
<p>Old-style callbacks are also supported. <em>Instead of</em> <code>.then()</code> you can call <code>.end()</code>:</p>
...
<p>The <strong>Node</strong> client supports making requests to <a href="http://en.wikipedia.org/wiki/Unix_domain_socket">Unix Domain Sockets</a>:</p>
...
<a class="sourceLine" id="cb25-4" title="4">    .<span class="at">serialize</span>(<span class="kw">function</span> <span class="at">serializer</span>(obj) <span class="op">{</span></a>
...
<p>This method has two optional arguments: number of retries (default 3) and a callback. It calls <code>callback(err, res)</code> before each retry. The callback may return <code>true</code>/<code>false</code> to control whether the request sould be retried (but the maximum number of retries is always applied).</p>
<pre><code> request
   .get(&#39;http://example.com/search&#39;)
   .retry(2) // or:
   .retry(2, callback)
   .then(finished);</code></pre>
...
<a class="sourceLine" id="cb29-12" title="12">   .<span class="at">sortQuery</span>(<span class="kw">function</span>(a<span class="op">,</span> b)<span class="op">{</span></a>
<a class="sourceLine" id="cb29-13" title="13">     <span class="cf">return</span> <span class="va">a</span>.<span class="at">length</span> <span class="op">-</span> <span class="va">b</span>.<span class="at">length</span><span class="op">;</span></a>
<a class="sourceLine" id="cb29-14" title="14">   <span class="op">}</span>)</a>
<a class="sourceLine" id="cb29-15" title="15">   .<span class="at">then</span>(callback)</a></code></pre></div>
...
<a class="sourceLine" id="cb34-3" title="3">  .<span class="at">end</span>(<span class="kw">function</span> (error<span class="op">,</span> res) <span class="op">{</span></a>
...
<ul>
<li><p><code>req.timeout({deadline:ms})</code> or <code>req.timeout(ms)</code> (where <code>ms</code> is a number of milliseconds &gt; 0) sets a deadline for the entire request (including all redirects) to complete. If the response isn’t fully downloaded within that time, the request will be aborted.</p></li>
<li><p><code>req.timeout({response:ms})</code> sets maximum time to wait for the first byte to arrive from the server, but it does not limit how long the entire download can take. Response timeout should be a few seconds longer than just the time it takes server to respond, because it also includes time to make DNS lookup, TCP/IP and TLS connections.</p></li>
</ul>
