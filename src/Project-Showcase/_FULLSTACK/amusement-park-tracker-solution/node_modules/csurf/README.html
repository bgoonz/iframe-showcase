...
<p><a href="https://npmjs.org/package/csurf"><img src="https://badgen.net/npm/v/csurf" alt="NPM Version" /></a> <a href="https://nodejs.org/en/download"><img src="https://badgen.net/npm/dm/csurf" alt="NPM Downloads" /></a> <a href="https://travis-ci.org/expressjs/csurf"><img src="https://badgen.net/travis/expressjs/csurf/master" alt="Build status" /></a> <a href="https://coveralls.io/r/expressjs/csurf?branch=master"><img src="https://badgen.net/coveralls/c/github/expressjs/csurf/master" alt="Test coverage" /></a></p>
...
<ul>
<li>If you are setting the <a href="#cookie">“cookie” option</a> to a non-<code>false</code> value, then you must use <a href="https://www.npmjs.com/package/cookie-parser">cookie-parser</a> before this module.</li>
...
<p>If you have questions on how this module is implemented, please read <a href="https://github.com/pillarjs/understanding-csrf">Understanding CSRF</a>.</p>
...
<div class="sourceCode" id="cb1"><pre class="sourceCode sh"><code class="sourceCode bash"><a class="sourceLine" id="cb1-1" title="1">$ <span class="ex">npm</span> install csurf</a></code></pre></div>
<h2 id="api">API</h2>
<!-- eslint-disable no-unused-vars -->
<div class="sourceCode" id="cb2"><pre class="sourceCode js"><code class="sourceCode javascript"><a class="sourceLine" id="cb2-1" title="1"><span class="kw">var</span> csurf <span class="op">=</span> <span class="at">require</span>(<span class="st">&#39;csurf&#39;</span>)</a></code></pre></div>
<h3 id="csurfoptions">csurf(<a href="#options">options</a>)</h3>
<p>Create a middleware for CSRF token creation and validation. This middleware adds a <code>req.csrfToken()</code> function to make a token which should be added to requests which mutate state, within a hidden form field, query-string etc. This token is validated against the visitor’s session or csrf cookie.</p>
<h4 id="options">Options</h4>
<p>The <code>csurf</code> function takes an optional <code>options</code> object that may contain any of the following keys:</p>
<h5 id="cookie">cookie</h5>
<p>Determines if the token secret for the user should be stored in a cookie or in <code>req.session</code>. Storing the token secret in a cookie implements the <a href="https://cheatsheetseries.owasp.org/cheatsheets/Cross-Site_Request_Forgery_Prevention_Cheat_Sheet.html#double-submit-cookie">double submit cookie pattern</a>. Defaults to <code>false</code>.</p>
<p>When set to <code>true</code> (or an object of options for the cookie), then the module changes behavior and no longer uses <code>req.session</code>. This means you <em>are no longer required to use a session middleware</em>. Instead, you do need to use the <a href="https://www.npmjs.com/package/cookie-parser">cookie-parser</a> middleware in your app before this middleware.</p>
<p>When set to an object, cookie storage of the secret is enabled and the object contains options for this functionality (when set to <code>true</code>, the defaults for the options are used). The options may contain any of the following keys:</p>
<ul>
<li><code>key</code> - the name of the cookie to use to store the token secret (defaults to <code>'_csrf'</code>).</li>
...
<li><code>secure</code> - marks the cookie to be used with HTTPS only (defaults to <code>false</code>).</li>
<li><code>maxAge</code> - the number of seconds after which the cookie will expire (defaults to session length).</li>
<li><code>httpOnly</code> - flags the cookie to be accessible only by the web server (defaults to <code>false</code>).</li>
<li><code>sameSite</code> - sets the same site policy for the cookie(defaults to <code>false</code>). This can be set to <code>'strict'</code>, <code>'lax'</code>, <code>'none'</code>, or <code>true</code> (which maps to <code>'strict'</code>).</li>
<li><code>domain</code> - sets the domain the cookie is valid on(defaults to current domain).</li>
</ul>
<h5 id="ignoremethods">ignoreMethods</h5>
<p>An array of the methods for which CSRF token checking will disabled. Defaults to <code>['GET', 'HEAD', 'OPTIONS']</code>.</p>
<h5 id="sessionkey">sessionKey</h5>
<p>Determines what property (“key”) on <code>req</code> the session object is located. Defaults to <code>'session'</code> (i.e. looks at <code>req.session</code>). The CSRF secret from this library is stored and read as <code>req[sessionKey].csrfSecret</code>.</p>
<p>If the <a href="#cookie">“cookie” option</a> is not <code>false</code>, then this option does nothing.</p>
<h5 id="value">value</h5>
<p>Provide a function that the middleware will invoke to read the token from the request for validation. The function is called as <code>value(req)</code> and is expected to return the token as a string.</p>
<p>The default value is a function that reads the token from the following locations, in order:</p>
<ul>
...
<p>The following is an example of some server-side code that generates a form that requires a CSRF token to post back.</p>
<div class="sourceCode" id="cb3"><pre class="sourceCode js"><code class="sourceCode javascript"><a class="sourceLine" id="cb3-1" title="1"><span class="kw">var</span> cookieParser <span class="op">=</span> <span class="at">require</span>(<span class="st">&#39;cookie-parser&#39;</span>)</a>
...
<p>Inside the view (depending on your template language; handlebars-style is demonstrated here), set the <code>csrfToken</code> value as the value of a hidden input field named <code>_csrf</code>:</p>
<div class="sourceCode" id="cb4"><pre class="sourceCode html"><code class="sourceCode html"><a class="sourceLine" id="cb4-1" title="1"><span class="kw">&lt;form</span><span class="ot"> action=</span><span class="st">&quot;/process&quot;</span><span class="ot"> method=</span><span class="st">&quot;POST&quot;</span><span class="kw">&gt;</span></a>
...
<a class="sourceLine" id="cb4-6" title="6"><span class="kw">&lt;/form&gt;</span></a></code></pre></div>
<h4 id="using-ajax">Using AJAX</h4>
<p>When accessing protected routes via ajax both the csrf token will need to be passed in the request. Typically this is done using a request header, as adding a request header can typically be done at a central location easily without payload modification.</p>
<p>The CSRF token is obtained from the <code>req.csrfToken()</code> call on the server-side. This token needs to be exposed to the client-side, typically by including it in the initial page content. One possibility is to store it in an HTML <code>&lt;meta&gt;</code> tag, where value can then be retrieved at the time of the request by JavaScript.</p>
<p>The following can be included in your view (handlebar example below), where the <code>csrfToken</code> value came from <code>req.csrfToken()</code>:</p>
<div class="sourceCode" id="cb5"><pre class="sourceCode html"><code class="sourceCode html"><a class="sourceLine" id="cb5-1" title="1"><span class="kw">&lt;meta</span><span class="ot"> name=</span><span class="st">&quot;csrf-token&quot;</span><span class="ot"> content=</span><span class="st">&quot;{{csrfToken}}&quot;</span><span class="kw">&gt;</span></a></code></pre></div>
<p>The following is an example of using the <a href="https://developer.mozilla.org/en-US/docs/Web/API/Fetch_API">Fetch API</a> to post to the <code>/process</code> route with the CSRF token from the <code>&lt;meta&gt;</code> tag on the page:</p>
<!-- eslint-env browser -->
<div class="sourceCode" id="cb6"><pre class="sourceCode js"><code class="sourceCode javascript"><a class="sourceLine" id="cb6-1" title="1"><span class="co">// Read the CSRF token from the &lt;meta&gt; tag</span></a>
...
<p>Many SPA frameworks like Angular have CSRF support built in automatically. Typically they will reflect the value from a specific cookie, like <code>XSRF-TOKEN</code> (which is the case for Angular).</p>
<p>To take advantage of this, set the value from <code>req.csrfToken()</code> in the cookie used by the SPA framework. This is only necessary to do on the route that renders the page (where <code>res.render</code> or <code>res.sendFile</code> is called in Express, for example).</p>
<p>The following is an example for Express of a typical SPA response:</p>
<!-- eslint-disable no-undef -->
<div class="sourceCode" id="cb7"><pre class="sourceCode js"><code class="sourceCode javascript"><a class="sourceLine" id="cb7-1" title="1"><span class="va">app</span>.<span class="at">all</span>(<span class="st">&#39;*&#39;</span><span class="op">,</span> <span class="kw">function</span> (req<span class="op">,</span> res) <span class="op">{</span></a>
...
<p><strong>Note</strong> CSRF checks should only be disabled for requests that you expect to come from outside of your website. Do not disable CSRF checks for requests that you expect to only come from your website. An existing session, even if it belongs to an authenticated user, is not enough to protect against CSRF attacks.</p>
<p>The following is an example of how to order your routes so that certain endpoints do not check for a valid CSRF token.</p>
<div class="sourceCode" id="cb8"><pre class="sourceCode js"><code class="sourceCode javascript"><a class="sourceLine" id="cb8-1" title="1"><span class="kw">var</span> cookieParser <span class="op">=</span> <span class="at">require</span>(<span class="st">&#39;cookie-parser&#39;</span>)</a>
...
<p>When the CSRF token validation fails, an error is thrown that has <code>err.code === 'EBADCSRFTOKEN'</code>. This can be used to display custom error messages.</p>
<div class="sourceCode" id="cb9"><pre class="sourceCode js"><code class="sourceCode javascript"><a class="sourceLine" id="cb9-1" title="1"><span class="kw">var</span> bodyParser <span class="op">=</span> <span class="at">require</span>(<span class="st">&#39;body-parser&#39;</span>)</a>
